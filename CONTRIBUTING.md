...
A harness that only supports plain-text injection needs no adapter code at all: point its hook at `devctl context`. Shipped adapters today: `antigravity`, `claude`, `cursor`, `grok`. Grok discards hook stdout on SessionStart, so that adapter also writes a managed `~/.grok/rules/devctl.md` (a standing instruction to run `devctl context`, not a live server snapshot, because Grok home rules apply to every project).
