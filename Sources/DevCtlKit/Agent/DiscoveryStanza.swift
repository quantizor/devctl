...
public enum DiscoveryStanza {
    ...
    public static func render(serverNames: [String]) -> String {
        ...
        return "- This project is registered with devctl (devservers.json). Prefer `devctl ensure \(example)` / `devctl status` / `devctl logs \(example)` over launching the server directly. Name servers after the project (not `web`); use `<project>.localhost` hosts, not bare `localhost`. In a git worktree, ensure still manages the server: the host stays the project's usual one and the port may be rebound; the live URL comes from `status` / session context. If context warns about a port conflict, follow that URL or hint; do not edit `devservers.json` just to change ports; do not start an unmanaged `pnpm dev --port`."
    }
}
