...
/** Detects a second devctl install shadowing the intended one.

    Two shapes, both hit in practice when a `make install` and a Homebrew cask
    coexist. A bare `devctl` runs the first match on PATH, and `~/.local/bin`
    ...
    Homebrew cannot overwrite a foreign `/Applications/devctl.app` it did not
    ...
    gathers those facts from disk for `devctl doctor`. */
public enum InstallShadow {
    /** A remediable shadowing between two devctl installs. */
    ...
    /** `foreignApp` is set only when an `/Applications/devctl.app` exists that
        ...
        `localDaemon` its sibling daemon. `pathWinner` is the devctl a bare
    ...
    public static func detect(
        ...
        if let homebrewCLI, let localCLI {
            let runs =
                pathWinner.map { "a bare `devctl` runs \($0)" }
                ?? "PATH order decides which one runs"
            let targets = [localCLI, localDaemon].compactMap { $0 }.joined(separator: " ")
            findings.append(
                Finding(
                    detail:
                        "two devctl CLIs are installed: Homebrew's at \(homebrewCLI) and a manual copy at \(localCLI), so \(runs) and a `brew upgrade` can change one while the other keeps running",
                    remedy: "rm \(targets)"))
        }
        if let foreignApp {
            findings.append(
                Finding(
                    ...
                    remedy: "brew reinstall --cask \(DevCtlDistribution.homebrewCaskToken)"))
        }
        return findings
    }
