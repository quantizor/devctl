...
/** The action a `directa://` URL requests. The raw value IS the URL authority
    (`directa://<verb>/…`), so parsing and serializing round-trip through it. */
...
/** A parsed `directa://` deep link. `head` is meaningful only for `open` (which
...
public struct DeepLink: Sendable, Equatable, Codable {
    ...
    private static func parse(components: URLComponents) -> Result<DeepLink, WireError> {
        guard components.scheme?.lowercased() == scheme else {
            return .failure(
                WireError(
                    code: .usage,
                    hint: "use a directa://<verb>/<project>/<server> URL",
                    message: "not a directa:// URL (scheme was '\(components.scheme ?? "")')"))
        }
        guard let host = components.host, !host.isEmpty else {
            return .failure(
                WireError(
                    code: .usage,
                    hint: "expected directa://open/<project>/<server>",
                    message: "directa:// URL is missing its verb"))
        }
        guard let verb = DeepLinkVerb(rawValue: host.lowercased()) else {
            return .failure(
                WireError(
                    ...
                    message: "unknown directa verb '\(host)'"))
        }
        ...
        var head: String?
        if segments.isEmpty {
            // Query-form alias: directa://<verb>?project=&server=&head=
            ...
            guard segments.count <= 3 else {
                return .failure(
                    WireError(
                        code: .usage,
                        hint: "shape is directa://\(verb.rawValue)/<project>/<server>[/<head>]",
                        message: "too many path segments in directa:// URL"))
            }
            slug = segments[0]
            server = segments[1]
        ...
        guard !server.isEmpty else {
            return .failure(
                WireError(code: .usage, message: "directa:// URL has an empty server name"))
        }
        if head != nil, verb != .open {
            return .failure(
                WireError(
                    code: .usage,
                    hint: "only directa://open/… takes a head",
    ...
    public static func resolveProject(
        ...
        case 0:
            return .failure(
                WireError(
                    code: .notFound,
                    hint: "run: directa status --all --json",
    ...
    public static let scheme = "directa"

    private static func validateSlug(_ slug: String) -> WireError? {
        if slug.isEmpty {
            return WireError(code: .usage, message: "directa:// URL has an empty project slug")
        }
    ...
    private static func missingSegments(verb: DeepLinkVerb) -> WireError {
        WireError(
            code: .usage,
            hint: "shape is directa://\(verb.rawValue)/<project>/<server>",
            message: "directa:// URL is missing its project and/or server")
    }
}
