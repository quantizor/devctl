...
/** One home for the strings that identify where directa is published and how it
    updates, shared by the app, the CLI, and the update check so a rename touches
    a single place. */
public enum DirectaDistribution: Sendable {
    ...
    /** The command an in-app control runs in Terminal to fully remove a cask
        install, including directa's own data via `zap`. */
