...
/** Checks GitHub for a newer release, sharing one on-disk cache between the app
    (which polls) and `directa doctor` (which reads the cache and refreshes only
    ...
    and the hook's output is meant for the servers, not directa itself. */
public enum UpdateCheck {
    ...
    static func cacheURL(paths: DirectaPaths) -> URL {
    ...
    public static func cachedStatus(
        paths: DirectaPaths = DirectaPaths(), currentVersion: String = DirectaVersion.version
    ) -> UpdateStatus? {
    ...
    public static func refreshIfStale(
        paths: DirectaPaths = DirectaPaths(), maxAge: TimeInterval = defaultMaxAge,
        currentVersion: String = DirectaVersion.version, now: Date = Date()
    ) async -> UpdateStatus? {
    ...
    public static func refresh(
        ...
        guard let url = URL(string: DirectaDistribution.latestReleaseAPIURL) else {
            return cachedFallback()
        }
        var request = URLRequest(url: url, timeoutInterval: 10)
        request.setValue("application/vnd.github+json", forHTTPHeaderField: "Accept")
        request.setValue("directa", forHTTPHeaderField: "User-Agent")
    ...
    private static func persist(_ cache: Cache, paths: DirectaPaths) {
    ...
    /** Tags are `vX.Y.Z`; the leading `v` is dropped so the string compares with
        the bare `DirectaVersion.version`. */
