...
- `AGENTS.md`'s codebase map lists `Switch`, `Lock`, `Link` as files under `Sources/devctl` and `PortGuard` under `Health/`; the first three are structs inside the ~1500-line `CLI.swift` and `PortGuard` sits in `HealthProber.swift`. Either fix the map or split those command structs into their own files (which also brings `CLI.swift` back under the size where splitting is worth asking about).
- `why <name>` gives no root cause for a *stopped* server whose port is held by another project's server: it answers "not running (stopped)" and stops, while the SessionStart summary for that same server already prints "port N held by '<other>' in <dir>". The diagnosis exists, and the root-cause command is the one place it does not surface, so rediscovering it took a manual `lsof` plus a `status --all` cross-reference. `why` should report a held port for stopped servers, not only for failed starts.
