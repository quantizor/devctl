...
- Registering from `/Applications` is not enough to keep the daemon off the volume. When the DMG is mounted, Launch Services can still resolve the agent (by shared bundle id) to the volume copy and spawn devctld from `/Volumes/devctl/...`, even though the BTM registration URL is `/Applications`. The running image then pins the volume, so it cannot be ejected until the daemon cycles. The daemon defends itself: at boot, before taking the single-instance lock, `DaemonImagePolicy` re-execs the canonical installed binary (`/Applications/.../Helpers/devctld`, else `~/.local/bin/devctld`) whenever its own image (`Bundle.main.executableURL`, symlinks resolved) is under `/Volumes`. A re-exec sentinel and a "run in place when no canonical binary exists" fallback keep it out of a KeepAlive respawn loop.
