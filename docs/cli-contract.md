...
- `devctl daemon install|uninstall [--purge]|start|stop|restart|status`: launchd lifecycle. `stop` drains and writes a deliberate-stop marker that auto-bootstrap honors; `restart` drains, kickstarts, and re-ensures what ran; `install` stages-and-renames the daemon binary and captures the login-shell PATH into the agent plist.
...
- `devctl hook install [--harness claude] [--statusline]`: idempotently wires a session-start hook into the harness's settings (claude: SessionStart with matcher `startup|resume|clear|compact` in ~/.claude/settings.json, emitting `hookSpecificOutput.additionalContext`). Adding a harness: CONTRIBUTING.md.
