...
- `devctl why <name> --json` → `{findings: [{server, phase, summary, evidence[]}], rootCause?}`. Walks `dependsOn` to the deepest broken dependency; evidence prefers `recentLogTail` / persisted `terminalEvidence`, else a structured-log window (out+err+sys). Exit 0 after a short life notes that controlled refusals are common. Agent session context never embeds raw child lines; it points here.
