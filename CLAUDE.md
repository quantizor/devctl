...
- Sources/DevCtlApp: menu bar app (DaemonModel 2s-polling model + crash notifications; PresenceLabel bakes colored dot+count pairs into an NSImage because MenuBarExtra labels flatten live color and render only one image; popover autogrows to a cap; nested head rows with UserDefaults-persisted pins; DashboardView logs/timeline/config tabs; SpotlightIndexer indexes servers and heads with config-icon thumbnails). Pure DaemonClient consumer.
