...
# directives run somewhere harmless, then uninstalls. Refuses to run if a devctl
# cask is already installed, and leaves nothing behind. It does NOT install to the
# real /Applications: devctl's own runtime behavior at the real path is covered by
...
# runs the real `devctl uninstall --agent-only`, so it will quit and unregister an
...
TAP="quantizor/tap"
TOKEN="quantizor/tap/devctl"
TAP_DIR="$(brew --repository)/Library/Taps/quantizor/homebrew-tap"
WORK="$(mktemp -d /tmp/devctl-cask-smoke.XXXXXX)"
...
if [[ "$INSTALL_TIER" == "1" ]]; then
  ...
  # run by reflex.
  if [[ "${DEVCTL_CASK_DESTRUCTIVE:-0}" != "1" ]]; then
    fail "the --install tier quits and unregisters your running app; re-run with DEVCTL_CASK_DESTRUCTIVE=1 to accept that"
  fi
  if brew list --cask devctl >/dev/null 2>&1; then
    fail "a devctl cask is already installed; refusing to run the destructive tier over it"
  fi
fi
...
SKIP_NOTARIZE=1 DEVCTL_DMG_QUARANTINE=0 make -C "$ROOT" dmg >/dev/null 2>&1 \
  || fail "make dmg failed; run it directly to see why"
VERSION="$("$ROOT/.build/release/devctl" --version 2>/dev/null || echo 0.0.0)"
DMG="$ROOT/dist/devctl-${VERSION}.dmg"
[[ -f "$DMG" ]] || fail "expected DMG at $DMG"
SHA="$(shasum -a 256 "$DMG" | awk '{print $1}')"
pass "built devctl-${VERSION}.dmg (sha ${SHA:0:12}…)"
...
/usr/bin/python3 - "$ROOT/packaging/homebrew/devctl.rb" "$TAP_DIR/Casks/devctl.rb" \
...
text = re.sub(
    r'url "[^"]*"',
    f'url "file://{dist}/devctl-#{{version}}.dmg"',
    text, count=1)
...
[[ -d "$APPDIR/devctl.app" ]] || fail "app not staged into --appdir"
...
CASK_APP="$(/bin/ls -d "$(brew --caskroom)/devctl"/*/devctl.app 2>/dev/null | head -1)"
[[ -n "$CASK_APP" ]] || fail "no Caskroom entry for devctl"
[[ "$(/usr/bin/readlink "$CASK_APP" 2>/dev/null || echo "")" == "$APPDIR/devctl.app" \
   || "$(cd "$CASK_APP" && pwd -P)" == "$(cd "$APPDIR/devctl.app" && pwd -P)" ]] \
...
BREW_CLI="$(brew --prefix)/bin/devctl"
...
xattr -dr com.apple.quarantine "$APPDIR/devctl.app" 2>/dev/null || true
...
# real `devctl uninstall --agent-only`, which quits and unregisters the app's
# agent on this machine (recoverable at next launch).
brew uninstall --cask --force "$TOKEN" >/dev/null 2>&1 || fail "brew uninstall"
[[ ! -e "$APPDIR/devctl.app" ]] || fail "app survived uninstall"
