...
# when /Applications/directa.app is also present. For SMAppService, install from
...
WORK="$(mktemp -d /tmp/directa-launchd-smoke.XXXXXX)"
...
cleanup() {
  "$DIRECTA" daemon uninstall > /dev/null 2>&1 || true
  rm -rf "$WORK"
}
trap cleanup EXIT
...
"$DIRECTA" daemon install --legacy > /dev/null || fail "daemon install --legacy"
pass "daemon install + hello (legacy home LaunchAgent)"

"$DIRECTA" daemon status | grep -q "pid" || fail "daemon status shows no pid"
pass "daemon status"

cd "$PROJECT"
"$DIRECTA" register --name web --cmd "$BIN/fixture-server" --json > /dev/null
"$DIRECTA" ensure web --timeout 15 --json > /dev/null || fail "ensure never went healthy"
PID_BEFORE="$("$DIRECTA" status web --json | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["servers"][0]["pid"])')"
pass "server up under launchd daemon (pid $PID_BEFORE)"

"$DIRECTA" daemon restart > /dev/null || fail "daemon restart"
PHASE="$("$DIRECTA" wait web --healthy --timeout 20 --json | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["server"]["phase"])')"
[[ "$PHASE" == "running" ]] || fail "server did not come back after restart (phase $PHASE)"
PID_AFTER="$("$DIRECTA" status web --json | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["servers"][0]["pid"])')"
...
"$DIRECTA" daemon install --legacy > /dev/null || fail "daemon install upgrade"
PHASE="$("$DIRECTA" wait web --healthy --timeout 20 --json | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["server"]["phase"])')"
[[ "$PHASE" == "running" ]] || fail "server did not come back after install upgrade (phase $PHASE)"
PID_POST_UPGRADE="$("$DIRECTA" status web --json | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["servers"][0]["pid"])')"
...
"$DIRECTA" daemon stop > /dev/null
sleep 2
set +e
STATUS_OUT="$("$DIRECTA" status web --json 2>&1)"
STATUS_EXIT=$?
set -e
...
"$DIRECTA" daemon start --legacy > /dev/null || fail "daemon start"
"$DIRECTA" daemon status | grep -q "pid" || fail "daemon not back after start"
pass "daemon start"
...
sleep 1
"$DIRECTA" status --json > /dev/null || fail "auto-bootstrap did not resurrect the daemon"
"$DIRECTA" daemon status | grep -q "pid" || fail "daemon not resurrected"
...
set +e
ALIAS_OUT="$("$DIRECTA" daemon uninstall --json 2>"$ALIAS_ERR")"
set -e
...
"$DIRECTA" daemon install --legacy > /dev/null || fail "reinstall before the agent-only test"
AGENT_ONLY_OUT="$("$DIRECTA" uninstall --agent-only --json 2>/dev/null)"
