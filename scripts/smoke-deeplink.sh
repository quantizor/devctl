#!/bin/zsh
# Launch Services E2E for directa:// (warm + cold open). Not part of default
# swift test; run before merging URL-scheme work. Requires a GUI session.
# Soft-fails the OSLog scrape when not on a tty unless DIRECTA_OSLOG_STRICT=1.
#
# Uses the LIVE default daemon socket (the menu bar app cannot see DIRECTA_SOCKET).
# Registers a throwaway server under a temp project, then tears it down.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BIN="$ROOT/.build/debug"
WORK="$(mktemp -d /tmp/directa-deeplink-smoke.XXXXXX)"
...
# `directa.app` would also match an unrelated `directaXapp`. Escape every dot in
...
APP_PATH="$ROOT/directa.app"
APP_MATCH="${APP_PATH//./\\.}/Contents/MacOS/directa-app"
...
phase_of() {
  "$DIRECTA" status "$SERVER" --project "$PROJECT" --json 2>/dev/null \
    | /usr/bin/python3 -c 'import json,sys; print(json.load(sys.stdin)["servers"][0]["phase"])' \
    2>/dev/null || echo missing
}

cleanup() {
  "$DIRECTA" stop "$SERVER" --project "$PROJECT" --json >/dev/null 2>&1 || true
  "$DIRECTA" unregister "$SERVER" --project "$PROJECT" --json >/dev/null 2>&1 || true
  [[ -n "${LOG_STREAM_PID:-}" ]] && kill "$LOG_STREAM_PID" 2>/dev/null || true
  # By bundle path, never by process name: the executable is directa-app for the
  # copy built here AND for the one the user has in /Applications, and killing
  # theirs is not this script's business. `killall DirectaApp` also matched
  # neither, so the copy this script launched used to outlive the run.
  pkill -f "$APP_MATCH" 2>/dev/null || true
  if [[ -d /Applications/directa.app ]]; then
    "$LSREGISTER" -f /Applications/directa.app >/dev/null 2>&1 || true
  fi
  rm -rf "$WORK"
}
trap cleanup EXIT

STRICT_OSLOG="${DIRECTA_OSLOG_STRICT:-}"
...
swift build --package-path "$ROOT" --product DirectaApp > /dev/null
...
[[ "$SCHEME" == "directa" ]] || fail "scheme was $SCHEME"
pass "Info.plist declares directa://"

# Need a live daemon on the default socket (menu bar app path).
"$DIRECTA" daemon status >/dev/null 2>&1 || fail "ddirecta not running on the default socket; run: directa daemon start"

cd "$PROJECT"
"$DIRECTA" register --name "$SERVER" --cmd "$BIN/fixture-server" --json > /dev/null
...
/usr/bin/log stream --style compact --predicate 'subsystem == "dev.quantizor.directa"' --level debug \
...
open -a "$APP" "directa://ensure/${SLUG}/${SERVER}"
PHASE=missing
for i in {1..40}; do
  PHASE="$(phase_of)"
  [[ "$PHASE" == "running" ]] && break
...
pkill -f "$APP_MATCH" 2>/dev/null || true
sleep 1
open -a "$APP" "directa://stop/${SLUG}/${SERVER}"
PHASE=missing
for i in {1..40}; do
  PHASE="$(phase_of)"
  [[ "$PHASE" == "stopped" ]] && break
