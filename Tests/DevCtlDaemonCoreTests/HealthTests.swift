...
private func makeSupervisor(
    ...
    port: Int? = nil
) throws -> (ServerSupervisor, DevCtlPaths, String) {
    let base = FileManager.default.temporaryDirectory.appending(path: "devctl-health-\(UUID().uuidString)")
    let project = base.appending(path: "proj")
    try FileManager.default.createDirectory(at: project, withIntermediateDirectories: true)
    let paths = DevCtlPaths(dataDir: base.appending(path: "data"), logsDir: base.appending(path: "logs"))
