...
/** The daemon must never keep its process image on a mounted volume: a DMG shares
    the app's bundle id, so Launch Services can spawn devctld from the volume copy,
...
@Suite struct DaemonImagePolicyTests {
    private let appHelper = "/Applications/devctl.app/Contents/Helpers/devctld"
    private let homeCLI = "/Users/x/.local/bin/devctld"
    private let volume = "/Volumes/devctl/devctl.app/Contents/Helpers/devctld"
    ...
    @Test func neverReExecsToAnotherVolumePathOrToItself() {
        ...
        #expect(
            DaemonImagePolicy.decide(
                currentExecutable: volume, candidates: [volume, "/Volumes/other/devctld"],
                alreadyReexeced: false, fileExists: { _ in true }) == .runHere)
    }

    @Test func mountedVolumeDetection() {
        #expect(DaemonImagePolicy.isUnderMountedVolume("/Volumes/devctl/x"))
        #expect(DaemonImagePolicy.isUnderMountedVolume("/Volumes"))
        #expect(!DaemonImagePolicy.isUnderMountedVolume("/Applications/devctl.app/x"))
        #expect(!DaemonImagePolicy.isUnderMountedVolume("/VolumesData/x"))
        #expect(!DaemonImagePolicy.isUnderMountedVolume("/Users/x/.local/bin/devctld"))
    }
}
