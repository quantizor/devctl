...
@Suite struct LegacyAgentPlistTests {
    ...
    @Test func exitTimeOutStaysUnderLaunchdCeiling() throws {
        ...
        let paths = DevCtlPaths(dataDir: dir)
        let plist = LaunchdAdmin.renderPlist(daemonPath: "/tmp/devctld", paths: paths)
...
@Suite struct AgentRebindPolicyTests {
    ...
    @Test func rebindMarkerRoundTrip() throws {
        let dir = FileManager.default.temporaryDirectory
            .appending(path: "devctl-rebind-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: dir) }
        let paths = DevCtlPaths(dataDir: dir)
...
@Suite struct DaemonRecoveryPolicyTests {
    ...
    /** A deliberate `devctl daemon stop` must survive the app's poll loop. */
